...
import { MockLedgity, MockUSDC, PancakeFactory, PancakePair, PancakeRouter, Reserve } from '../typechain';
...
describe('Reserve', () => {
  ...
  snapshottedBeforeEach(async () => {
    ...
    ).deploy(router.address, token.address, usdcToken.address, timelock);
  ...
  describe('constructor', () => {
    it('should not allow zero address timelock', async () => {
      ...
      await expect(
        (
          await ethers.getContractFactory('Reserve')
        ).deploy(router.address, tokenA.address, tokenB.address, ZERO_ADDRESS),
  ...
  describe('swapping for USDC', () => {
    ...
    it('should not allow anyone except the token contract to swap the tokens', async () => {
      for (const sender of [aliceAccount, bobAccount]) {
        await expect(reserve.connect(sender).swapAndCollect('10')).to.be.revertedWith(
          'Reserve: caller is not the token',
        );
      }
    });
  });
  ...
  describe('swap and liquify', () => {
    ...
    async function testSwapAndLiquify({
      reserveBalance,
      amount,
      liquified,
    }: {
    ...
    it('should not allow anyone except the token contract to swap and liquify tokens', async () => {
      for (const sender of [aliceAccount, bobAccount]) {
        await expect(reserve.connect(sender).swapAndLiquify('10')).to.be.revertedWith(
          'Reserve: caller is not the token',
        );
      }
    });
