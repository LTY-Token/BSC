...
export async function addLiquidityUtil(
  tokenAmountWithoutDecimals: BigNumberish,
  usdcAmountWithoutDecimals: BigNumberish,
  token: ILedgity,
  usdcToken: MockUSDC,
...
export async function deployPancakeswap(signer: Signer) {
  const PancakeFactory = new ethers.ContractFactory(
    PancakeFactoryArtifact.abi,
    PancakeFactoryArtifact.bytecode,
    signer,
  ) as PancakeFactory__factory;
  const PancakeRouter = new ethers.ContractFactory(
    PancakeRouterArtifact.abi,
    PancakeRouterArtifact.bytecode,
    signer,
